...
const HomePage = ({ data }) => {
  function paragraphStr(data) {
    const htmlStr = data.map((item) => {
      ...
      const modifiedHtml =
        originalHtml.slice(0, 2) + ' class="mt-6"' + originalHtml.slice(2);
        ...
        <div className='mx-auto grid max-w-2xl grid-cols-1 gap-x-8 gap-y-16 sm:gap-y-10 lg:mx-0 lg:max-w-none lg:grid-cols-2'>
          <div className='lg:pr-8 lg:pt-4'>
            <div className='lg:max-w-lg'>
              ...
              <div
                dangerouslySetInnerHTML={paragraphsElement}
                className='mt-6 text-lg leading-8 text-gray-600'
              />
              <div className='flex xl:gap-10 md:gap-8 sm:gap-4 gap-2 items-center justify-between py-6'>
                {data.buttons.map((button) => (
                  <Link
                    key={button.id}
                    href={`${button.page ? button.page.slug : '#'}`}
                    className='text-sm font-semibold leading-6 text-gray-900 w-full'
                  >
                    <Button className='w-full'>{button.name}</Button>
                  </Link>
                ))}
              </div>
            </div>
